#!/usr/bin/env python3
"""
WER (Word Error Rate) benchmark for OpenAI Whisper Large v3 ASR model.
...
def main():
    ...
    for i, (txt_path, vocal_path) in enumerate(pairs, 1):
        ...
        try:
            ...
            # Transcribe with Whisper (return_timestamps=True required for long audio)
            output = pipe(
                vocal_path,
    ...
    if results:
        logger.info("\n" + "=" * 70)
        logger.info("WER BENCHMARK RESULTS (Whisper Large v3)")
        logger.info("=" * 70)
