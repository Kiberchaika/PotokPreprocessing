#!/usr/bin/env python3
"""
WER (Word Error Rate) benchmark for Qwen3 ASR model.
...
def find_paired_files(directory: str) -> list[tuple[str, str]]:
    ...
    for root, _, files in os.walk(directory):
        ...
        for vocal_file in vocal_files:
            # Remove '_vocal.mp3' suffix properly
...
def main():
    ...
    for i, (txt_path, vocal_path) in enumerate(pairs, 1):
        ...
        try:
            ...
            # Transcribe audio
            transcription_results = model.transcribe(
                audio=vocal_path,
                language="Russian",
            )

            if not transcription_results or not transcription_results[0].text:
                logger.warning("  Empty transcription, skipping")
                continue

            hypothesis = transcription_results[0].text
            ...
            # Flag hallucination if hypothesis is much longer than reference
    ...
    if results:
        logger.info("\n" + "=" * 70)
        logger.info("WER BENCHMARK RESULTS (Qwen3 ASR)")
        logger.info("=" * 70)
